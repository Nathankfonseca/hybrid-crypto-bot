...
def train_and_test_15m():
    print("Loading 15m data...")
    df = pd.read_parquet('data/micro_15m.parquet')
    df.sort_values('timestamp', inplace=True)
    
    df['log_return'] = np.log(df['close'] / df['close'].shift(1))
    df['momentum'] = df['close'] - df['close'].shift(1)
    
    # Predict the return of the VERY NEXT 15m candle
    ...
    # Last 60 days = 60 * 24 * 4 = 5760 bars
    oos_bars = 5760
    ...
    print(f"15m MLP Accuracy: {acc*100:.2f}%")
    ...
    # If we buy and hold for 1 candle:
    pnl = 0.0
    trades = 0
